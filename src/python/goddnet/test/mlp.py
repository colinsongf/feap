...
class MLP(object):
    ...
    def __init__(self, rng, input, n_in, n_hidden, n_out):
        ...
        #    n_in=n_hidden,
        #    n_out=n_out)
        self.outLayer = HiddenLayer(rng=rng, input=self.hiddenLayer.output, n_in=n_hidden, n_out=n_out, activation=T.tanh)
        ...
        #self.negative_log_likelihood = self.outLayer.negative_log_likelihood
    ...
    def cost(self, target):
        return T.sum(T.mean(.5*T.sqr(T.sub(target, self.outLayer.output))))

    def errors(self, y):
        return T.mean(T.sqrt((T.sqr(T.sub(self.outLayer.output,y)))))


def test_mlp(learning_rate=0.2, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
             dataset='../../../data/cifar-10-batches-py', batch_size=20, n_hidden=500):
   ...
   """
    ...
    datasets = load_cifar10_data(dataset)
    ...
    #classifier = MLP(rng=rng, input=x, n_in=28 * 28, n_hidden=n_hidden, n_out=1)
    classifier = MLP(rng=rng, input=x, n_in=3072, n_hidden=n_hidden, n_out=10)
